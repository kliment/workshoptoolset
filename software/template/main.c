...
void pid_harder(){
    ...
    total_error += error;
    if(total_error>maxduty*1.0)total_error=maxduty*1.0;
    else if(total_error<0.0)total_error=0.0;
    double delta_error = error - prev_error;
    int32_t pduty = Kp*error;// + (Ki*100.0)*total_error + (Kd/100.0)*delta_error; //PID control compute
    if (pduty > maxduty) pduty=maxduty;
    else if (pduty<0) pduty=0;
    duty=pduty;
    prev_error=error;
}
